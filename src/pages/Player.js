...
class Player extends React.Component {
    ...
    controles = () => this.setState({controles: true, play: true})
    ...
    download = url => fileSaver.saveAs(url)

    ref = player => this.player = player

    carregaPlayer = () => {
        let {play} = searchEmJSON(this.props.location.search)
        play = atob(play)
        const {imagem, tema, base, audio, data, duracao} = JSON.parse(play)
        this.setState({imagem: imagem, titulo: tema, duracao: duracao, data: data, descricao: base, audio: audio})
    }
    ...
    render() {
        ...
        const {imagem, titulo, duracao, data, descricao, audio} = this.state
                ...
                <div id="card-player">
                    ...
                    <div id="card-descritivo">
                        <div id="descricao-player">
                            ...
                            <FormLabel
                                id="descricao-player">{`${duracao} - ${data}`}</FormLabel>
                        </div>
                        <div>
                ...
                </div>
                {
                    ...
                    <div id="footer">
                        <div id="footer-left">
                            ...
                            {
                                !this.state.play ?
                                    <PlayArrowRoundedIcon id="icon-play-pause" onClick={this.playPause}/> :
                                    <PauseRoundedIcon id="icon-play-pause" onClick={this.playPause}/>
                            }
                        ...
                        <div id="footer-right">
                            <div id="seek">
                                <FormLabel id="time">{<Duration
                                    seconds={this.state.duration * this.state.played}/>}</FormLabel>
                                <input
                                    id="progress"
                                ...
                                <FormLabel id="time">{<Duration
                                    seconds={this.state.duration * (1 - this.state.played)}/>}</FormLabel>
                            </div>
                            <div id="volume">
                                <VolumeDownRoundedIcon id="icon-play-pause"/>
                                <input id="progress" type='range' min={0} max={1} step='any' value={this.state.volume}
